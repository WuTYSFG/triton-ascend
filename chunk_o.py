...
def chunk_bwd_kernel_dv_local(
    ...
    i_t_global, i_b = tl.program_id(0), tl.program_id(1)
    T_max = T
    if IS_VARLEN:
        i_n, i_t_local = tl.load(chunk_indices + i_t_global * 2).to(tl.int32), tl.load(chunk_indices + i_t_global * 2 + 1).to(tl.int32)
        bos, eos = tl.load(cu_seqlens + i_n).to(tl.int32), tl.load(cu_seqlens + i_n + 1).to(tl.int32)
        T = eos - bos
    else:
        i_t_local = i_t_global
        bos, eos = i_b * T, i_b * T + T
    ...
    for i_h in range(HV):
        ...
        if USE_A:
            p_A = tl.make_block_ptr(A + (bos * HV + i_h) * BT, (BT, T), (1, HV*BT), (0, i_t_local * BT), (BT, BT), (0, 1))
            b_A = tl.load(p_A, boundary_check=(0, 1))
        else:
            if USE_G:
                ptr_g = g + (i_b * HV + i_h) * T_max + o_t
            ...
            b_A = tl.zeros([BT, BT], dtype=tl.float32)
            for i_k in range(tl.cdiv(K, BK)):
                p_k = tl.make_block_ptr(k_head, (T, K), (H*K, 1), (i_t_local * BT, i_k * BK), (BT, BK), (1, 0))
                p_q = tl.make_block_ptr(q_head, (K, T), (1, H*K), (i_k * BK, i_t_local * BT), (BK, BT), (0, 1))
        ...
        m_A = (o_t[:, None] <= o_t[None, :]) & (m_t[:, None] & m_t)
        b_A = tl.where(m_A, b_A, 0).to(do_head.dtype.element_ty)

        for i_v in range(tl.cdiv(V, BV)):
            p_do = tl.make_block_ptr(do_head, (T, V), (HV*V, 1), (i_t_local * BT, i_v * BV), (BT, BV), (1, 0))
            p_dv = tl.make_block_ptr(dv_head, (T, V), (HV*V, 1), (i_t_local * BT, i_v * BV), (BT, BV), (1, 0))
            b_do = tl.load(p_do, boundary_check=(0, 1))
            b_dv = tl.dot(b_A.to(b_do.dtype), b_do)
            tl.store(p_dv, b_dv.to(p_dv.dtype.element_ty), boundary_check=(0, 1))
